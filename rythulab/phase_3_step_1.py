		...
		recommendations.append(
			{
				...
				"canopy_layer_class": crop.get("canopy_layer_class") or "",
				"root_depth_class": crop.get("root_depth_class") or "",
				"fills": fills,
				"reasons": reasons,
...
def build_frontend_gap_payload(crop_ids: List[str]) -> Dict[str, Any]:
	"""
	Frontend adapter for Phase 3 Step 1/2.
	Returns gaps in CS3.gaps-compatible shape plus metadata labels from backend.
	"""
	coverage_result = analyze_biodiversity_coverage(crop_ids)
	gap_fill_result = recommend_crops_for_biodiversity_gaps(crop_ids)
	coverage = coverage_result.get("coverage", {})

	functional = coverage.get("functional_group", {})
	family = coverage.get("family", {})
	canopy = coverage.get("canopy_layer_class", {})
	root = coverage.get("root_depth_class", {})

	covered_groups = {
		group_label: True
		for group_label in (functional.get("covered") or [])
	}
	families_map = {
		family_label: True
		for family_label in (family.get("covered") or [])
	}

	return {
		"selected_crop_ids": coverage_result.get("selected_crop_ids", []),
		"known_crop_ids": coverage_result.get("known_crop_ids", []),
		"unknown_crop_ids": coverage_result.get("unknown_crop_ids", []),
		"gaps": {
			"missingGroups": functional.get("not_covered", []),
			"families": families_map,
			"missingLayers": canopy.get("not_covered", []),
			"missingRoots": root.get("not_covered", []),
			"coveredGroups": covered_groups,
			"allGroups": functional.get("all_values", []),
			"allLayers": canopy.get("all_values", []),
			"allRoots": root.get("all_values", []),
		},
		"coverage": coverage,
		"recommended_crops": gap_fill_result.get("recommended_crops", []),
	}
